...
#define PIC1_OCW2			0xa0// OCW2寄存器的第2位是EOI命令的输出端
...
void irq_init (void);//初始化中断
void do_handler_divider(exception_frame_t *frame);//处理除0异常
typedef void (*irq_handler_t)(exception_frame_t *frame);//中断处理函数类型定义
void exception_handler_divider(void);
int irq_install(int irq_num, irq_handler_t handler);//安装中断处理函数
...
irq_state_t irq_enter_protection(void);//进入临界区保护模式
void irq_leave_protection(irq_state_t state);//退出临界区保护模式
