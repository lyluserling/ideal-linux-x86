...
do_default_handler(frame, "general_protection Error.");
}

void do_handler_page_fault(exception_frame_t * frame) {
do_default_handler(frame, "page_fault Error.");
...
// 	outb(PIC0_ICW1, PIC_ICW1_ALWAYS_1 | PIC_ICW1_ICW4);
// 	outb(PIC0_ICW2, IRQ_PIC_START);
// 	outb(PIC0_ICW3, 1 << 2); //PIC0_ICW3第二位设置为1
// 	outb(PIC0_ICW4, PIC_ICW4_8086);

// 	outb(PIC1_ICW1, PIC_ICW1_ALWAYS_1 | PIC_ICW1_ICW4);
// 	outb(PIC1_ICW2, IRQ_PIC_START+8);
// 	outb(PIC1_ICW3, 1 << 2); //PIC0_ICW3第二位设置为1
// 	outb(PIC1_ICW4, PIC_ICW4_8086);

// 	outb(PIC0_IMR, 0xFF & ~(1 << 2)); //禁止所有中断, 但允许从PIC1传来的中断 不禁止IRQ2
...
int irq_install(int irq_num, irq_handler_t handler){
    ...
    gate_desc_set(idt_table + irq_num, KERNEL_SELECTOR_CS,
                 (uint32_t)handler, GATE_P_PRESENT | GATE_DPL0 | GATE_TYPE_IDT);//设置中断描述符表
     return 0;
}

void irq_enable(int irq_num) {
	if(irq_num < IRQ_PIC_START){//如果是从0到7号中断
		return;
	}
	irq_num -= IRQ_PIC_START;
	if(irq_num < 8){//如果是从0到7号中断
		uint8_t mask = inb(PIC0_IMR) & ~(1 << irq_num);//获取PIC0_IMR的第irq_num位   & ~(1 << irq_num):清零
		outb(PIC0_IMR,mask);//重新设置PIC0_IMR
	}else{
		irq_num -= 8;//如果是从8到15号中断
		uint8_t mask = inb(PIC1_IMR) & ~(1 << (irq_num));//获取PIC1_IMR的第irq_num位
		outb(PIC0_IMR,mask);//重新设置PIC0_IMR
	}
}
...
void irq_disable(int irq_num) {
	...
	irq_num -= IRQ_PIC_START;
	if(irq_num < 8){//如果是从0到7号中断
		uint8_t mask = inb(PIC0_IMR) | ~(1 << irq_num);//获取PIC0_IMR的第irq_num位   置1
		outb(PIC0_IMR,mask);//重新设置PIC0_IMR
	}else{
		irq_num -= IRQ_PIC_START;
		uint8_t mask = inb(PIC1_IMR) | ~(1 << (irq_num));//获取PIC1_IMR的第irq_num位
